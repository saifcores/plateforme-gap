...
import com.uchk.gap.administration.service.CourrierService;
...
public class CourrierController {
    ...
    @GetMapping("/{id}")
    public CourrierDto findById(@PathVariable Long id) {
        return service.findById(id);
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public CourrierDto create(@Valid @RequestBody CourrierDto.Request request) {
        return service.create(request);
    }

    @PutMapping("/{id}")
    public CourrierDto update(@PathVariable Long id, @Valid @RequestBody CourrierDto.Request request) {
